...
namespace K {
  class MG: public Klass {
	...
	{
	    try{
		const string mkapiUrl = "https://bittrex.com/api/v1.1/public/getticker?market=BTC-LTC"; 
		string mkjson = getOrderBook(mkapiUrl);
		double mkAsk=getJsonValue("Ask", mkjson), mkBid=getJsonValue("Bid", mkjson), mkFv= (mkAsk + mkBid)/2;			
	...
	//((SH*)screen)->log("DB", to_string(mkFv) + string(" BID ") + to_string(mkBid) + string(" ASK ") + to_string(mkAsk) + string("--------INSERTed into ORDERBOOK----\n"));
	}catch(...)
	{
		((SH*)screen)->log("DB", string("--------UNABLE to make ORDERBOOK----\n"));
	 }
	}*/
      };
	...
	//populate orderbook
	 mLevels populatelevel()
	{      
	vector<mLevel> sonalibid;
	vector <mLevel> sonaliask;
	for(int i=0; i < 10; i++)
	{
		sonalibid.push_back(mLevel());
		sonaliask.push_back(mLevel());
	}
	...
	sonaliprices(sonaliask, "askprice");//? cout<<"UPDATED":cout<<"\nNOTTTT\n";
	sonaliprices(sonalibid, "bidprice");//? cout<<"UPDATED":cout<<"\nNOTTTT\n";;
	paluchisizes(sonaliask, "asksize");//? cout<<"UPDATED":cout<<"\nNOTTTT\n";;
	paluchisizes(sonalibid, "bidsize");//? cout<<"UPDATED":cout<<"\nNOTTTT\n";;

	mLevels paluchi(sonalibid, sonaliask);
	return paluchi;
}
	bool sonaliprices(vector<mLevel> &sonali, string filename)
      {
        double x;
        int i=0;
        ifstream inFile;
       ...
       bool paluchisizes(vector<mLevel> &paluchi, string filename)
      {
        double x;
        int i=0;
        ifstream inFile;
        ...
        inFile.open(filename);
        if (!inFile) {
            cout << "unable to gET ORDERBOOK "<<filename<<endl;
            createTrades();
	    paluchisizes(paluchi, filename);
        }
