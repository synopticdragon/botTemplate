...
namespace K {
  ...
  class SH {
	...
	createTrades();
      ...
      void log(mTrade k, string e , string orderid) {
          string tradeside, tradeqnty,tradecrncy;
        /*sendFixPeatio( k.side == mSide::Bid ? tradeside='1' : tradeside='2', to_string(k.price),
          to_string(k.quantity), 
          trTransactTime(),
          ((k.pair.base).append(k.pair.quote)),
          k.tradeId);*/
	  cancelsendFixPeatio(orderid, (k.side == mSide::Bid ? tradeside='1' : tradeside='2'), trTransactTime(),to_string(k.price), to_string(k.quantity), ((k.pair.base).append(k.pair.quote)));
