  ...
  class EV: public Klass {
    ...
    protected:
      void load() {
        ...
        signal(SIGUSR1, wtf);
#endif
        tracelog = "- upstream: " + exName/*((CF*)config)->argExchange*/ + '\n'
    ...
    private:
      ...
      static void quit(const int sig) {
        tracelog = string("Excellent decision! ") + string(" SONALI PALUCHI")              
          + '\n';
        halt(EXIT_SUCCESS);
      };
      static void wtf(const int sig) {
        const string rollout = tracelog;
        tracelog = string(RCYAN) + "Warning: Signal " + to_string(sig) + ' '
#ifndef _WIN32
          + strsignal(sig)
#endif
          ;
        if (FN::output("test -d .git && git rev-parse @") != FN::output("test -d .git && git rev-parse @{u}"))
          tracelog += string(" API FAILURE).") + '\n';
            
        else {
          tracelog += string(" API FAILURE!!:") + '\n' + rollout
            + '\n'
#ifndef _WIN32
            + "--->Its  "/*- os-uname: " + FN::output("uname -srvm")*/
            + " Okay: " + '\n';
          /*void *k[69];
...
#endif
          ;
          tracelog += '\n' + string(BRED) + "DoNt PaNic!! Every little thing, is gonna be Alright." + string(RRED)
            + '\n' + "Take a vacation to https://www.visitjamaica.com"
            + '\n' + '\n';
        }
        halt(EXIT_FAILURE);
      };
