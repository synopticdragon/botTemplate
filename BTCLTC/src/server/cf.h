...
namespace K {
  class CF: public kLass {
    ...
    mAmount argMaxWallet    = 0;
     ...
     string argTitle        = "K.sh", argMatryoshka   = "https://www.visitjamaica.com/",
            argUser         = "NULL", argPass         = "NULL",
            argExchange     = "BeaXchange", argCurrency     = "NULL",
      ...
      inline void tidy() {
        ...
        if (argDatabase.empty() or argDatabase == ":memory:")
          (argDatabase == ":memory:"
            ...
            + '.' + "BEAXCHANGE" //FN::strU(argExchange)
            + '.' + base()
            + '.' + quote()
            + '.' + "db";
        argMaxLevels = argMaxLevels
          ? max(15, argMaxLevels)
          : 20;//321; //SONALI PALUCHI
