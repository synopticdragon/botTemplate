...
namespace K {
  class PG: public Klass {
	...
	k.base.amount=5.10; 
	if(((getTime() % 6) ==0) && (counter == 2)){}else if(((getTime() % 6) !=0) && (counter ==2)){counter =1;}
	if(((getTime() % 6) ==0) && (counter ==1))
	{
		makePaluchi();
		counter=2;			
	}
	k.quote.amount = k.base.amount * getSonali("currentprice");                 
	calcWallet(k);
        };
	...
	double getSonali(string filename)
	{
		double x, pricer;
		int i=0;
		ifstream inFile;
		...
		if (!inFile) {
		    ...
		    makePaluchi(); getSonali(filename);
		}
    ...
    public:
      ...
      function<void()> calcTargetBasePos = [&]() {                  _debugEvent_
        ...
        if (targetBasePosition and abs(targetBasePosition - next) < 1e-5 and sideAPR_ == sideAPR) return;       
         next= 4.60768;
       ...
       // ((DB*)memory)->insert(mMatter::TargetBasePosition, k);
        ...
        ((SH*)screen)->log("PG", string(" Calculating tradepositon against basevalue\n TBP: ")
